...
$result1 = dbquery($con1,("select * from userinfo_copy where title='1' limit 10;"));
...
list($use1, $date1, $Date2, $mo, $d8, $d8f, $yr)=getarray($con1, ("SELECT dtr, Date1, Date2, DATE_FORMAT(Date1, '%M'), DATE_FORMAT(Date1, '%e'), DATE_FORMAT(Date2, '%e'), DATE_FORMAT(Date2, '%Y') FROM dtrme ORDER BY id DESC LIMIT 1;"));
$date=$mo;
$me=$d8;
$me1=$d8f;
$me2=$yr;
...
list($name)=getarray($con1, ("select name from userinfo_copy where id='$use';"));
...
<br>
 <p style="padding-left: 140px;  font-family:arial; font-size: 18px;">'.str_repeat("&nbsp;",17).''.$name.'<u>
...
<p style="text-align:left; padding-top:-10px; font-family:arial; font-size:-1px"> '.str_repeat("&nbsp;",49).''.str_repeat("_",20).'</p>
<p style="text-align:left; padding-left:25px; font-family:arial; font:size:10px">'.str_repeat("&nbsp;",49).'<b><i>In Charge</b></p>
<p style="text-align:left; padding-left:25px; font-family:arial; font:size:8px">(See instructions on back)</p>
<columnbreak/>';
