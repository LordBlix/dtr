...
$yr='2018';
  ...
  $sql      = "SELECT DATE id, DATE model_no, clock1 mobile_name, clock2 price, clock3 company, clock4 mobile_category, clock4 1price FROM (
    ...
    WHERE MONTH(DATE) = MONTH('$mo') AND DATE BETWEEN '$mo' AND '$m1' AND DATE NOT IN (SELECT DISTINCT 
    CAST(checktime AS DATE) DATE FROM checkinout WHERE userid='$use'
     AND CAST(checktime AS DATE) BETWEEN '$mo' AND '$m1')) c) e ON b.DATE=e.DATE WHERE
      B.DATE NOT IN (SELECT TRIM(LEADING 0 FROM DATE_FORMAT(CAST(checktime AS DATE),'%d')) DATE FROM checkinout g
        
    WHERE CAST(g.checktime AS DATE) BETWEEN '$mo' AND '$m1' AND userid='$use' 
...
list($d8f)=mysqli_fetch_array(mysqli_query($con1,$n3));
    ...
    <p style="padding-left: 10px; font-family:arial; font-size: 12px;"> For the month of ______<u><b>April 1 - 30, 2018</b></u>_____<br> 	
...
<p style="text-align:left; font-family:arial; font-size:10px">'.str_repeat("&nbsp;",50). ' '.str_repeat("-",50).'</p>
<p style="text-align:left; padding-left:25px; font-family:arial; font:size:10px">(See instructions on back)'.str_repeat("&nbsp;",9).'<b><i>In Charge</i></b></p>
      ...
      <html>';
       //echo $html; die;  
      $path       = 'upload/';
      $file_name ="Roche Solutions-".$name.".pdf";
...
Official hours for arrival <?php echo str_repeat("&nbsp;", 3); ?> {Regular days<?php echo str_repeat("_",10)?><br>
<?php echo str_repeat("&nbsp;", 8); ?>and departure 
<?php echo str_repeat("&nbsp;", 14); ?>{Saturdays<?php str_repeat("_",12)?></p>
</EM></CAPTION>
  </head>
...
<th><b><br>Hours</b><th><b>Min-<br>utes</b>
        ...
        <br>
        <input type="submit" name="submit" name="submit1" class="pull-right btn btn-warning btn-large" style="margin-right:40px" value="Genarte PDF">
        <br>
        <tbody>
            <?php
            if (mysqli_num_rows($result) > 0 ) {
                while($row = mysqli_fetch_array($result)){
                    ?>
<tr class="pdf_border">
<td width="10%" style="border-top: 0.1mm solid black; border-bottom: 0.1mm solid black; border-left: 0.1mm solid black; padding-left: 10px;"><?php echo $row['model_no']; ?></td>   
...
<p style="text-align:left; font-family:arial font-size: 10px;"><?php echo str_repeat("&nbsp;", 14); ?><b>TOTAL</b><?php echo str_repeat("_",20)?></p>
...
of the hours of work performed,<br> record of which was made daily at
the time  of arrival <br> and departure from office.</i> </p>
<table><p style="padding-top:-210px; text-align:left; font-family:arial; font-size: 10px"><?php echo str_repeat("&nbsp;", 32)?><?php str_repeat("_",35)?></p></table>
<p style="text-align:left; padding-left:20px; font-family:arial; font:size:10px">Verified as to the prescribed office hours.</p>
