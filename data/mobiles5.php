...
mysql_select_db('lgu') or die ('cannot connect to the database');

$fan="select LAST_DAY('$date1')";
$res=mysql_query($fan);
list($fan1)=mysql_fetch_array($res);


$CreateSql = "INSERT INTO `dtrme` (`Date1`,`Date2`) VALUES ('$date1','$date2')";
...
function getarray($con1, $query){
 ...
 // $mpdf=new mPDF('win-1252','Legal','','',18,15,12,10,16,19);
 //$mpdf=new mPDF('win-1252','Legal','','',18,15,12,10,16,19);  keeep this
 $mpdf=new mPDF('win-1252','Legal','','',18,15,12,10,16,19);
...
//$result1 = dbquery($con1,("select * from userinfo_copy where title='$tala' and ID in ('133','136');"));
//$result1 = dbquery($con1,("select * from userinfo_copy where title='$tala' and ID in ('31', '20', '27', '22', '27', '19', '21', '17', '104', '89','29','18') limit 16;"));
...
$ff="17";
list($name,$head, $head1, $ee, $ff, $fs, $spc0)=getarray($con1, ("SELECT NAME, b.Head1, b.Head2, b.spc,
SSN, b.fs, b.spc0 FROM userinfo_copy a LEFT JOIN dep_head b ON  a.Deptcode=b.ID WHERE a.id='$use';"));
//list($ee, $head, $head1, $ee $ff, $fs, $spc0)=getarray($con1, ("select name, Name1, Head2, spc, SSN, fs, spc0 FROM userinfo_copy where id='$use';"));
...
<br>
 ...
 '.str_repeat("&nbsp;",8).' and departure '.str_repeat("&nbsp;", 23).' {Saturdays'.str_repeat("_",10).'
    ...
    <p style="text-align:justify; font-family:arial; font-size: 13px;">'.str_repeat("&nbsp;",5).'I <i> CERTIFY <i> on <i> my
    <i> honor that the above is a true and correct report
	  of the hours of work performed,<i> record of which was made daily at
