...
$result1 = dbquery($con1,("SELECT * FROM userinfo_copy a LEFT JOIN checkinout b 
ON a.ID=b.USERID WHERE Deptcode=$ID AND TITLE=2 AND CAST(CHECKTIME AS DATE) 
BETWEEN '2018-12-01' AND '2018-12-31' group by a.id;"));
...
$result = dbquery($con1,("SELECT DATE id, DATE model_no, clock1 mobile_name, clock2 price, clock3 company, clock4 mobile_category, clock4 1price FROM (
    SELECT b.DATE, e.clock1  Clock1, b.clock2, b.clock3, b.clock4 FROM (SELECT TRIM(LEADING 0 FROM DATE_FORMAT(CAST(DATE AS DATE),'%d')) DATE, '' clock1, '' clock2, '' clock3, '' clock4
    FROM
    (
        SELECT
...
<tbody>';
                            ...
                            <tr>
                            <td align = "center" colspan="7" style="font-size:13px; ">I  CERTIFY on my honor that  the  above  is  a  true  and correct report of the hours of work performed, record of which was made daily at the  time of arrival and departure from office.
                            <tr>
