...
//$result1 = dbquery($con1,("select * from userinfo_copy where title='$tala' limit 2;"));
$result1 = dbquery($con1,("select * from userinfo_copy where  ID=$ID;"));
...
$result = dbquery($con1,("SELECT DATE id, DATE model_no, clock1 mobile_name, clock2 price, clock3 company, clock4 mobile_category, clock4 1price FROM (
    ...
    AND ID IN (
SELECT MAX(id) FROM checkinout WHERE userid='$use' AND  CAST(checktime AS TIME) 
between'$amini' AND '$aminf' AND checktype='$ckami' AND CAST(checktime AS DATE) 
BETWEEN '$date1' AND '$Date2' GROUP BY CAST(checktime AS DATE)  
 )
    ...
    AND ID IN (
SELECT MAX(id) FROM checkinout WHERE userid='$use' AND  CAST(checktime AS TIME) BETWEEN '$amouti' AND '$amoutf' 
AND checktype='$ckamo' AND CAST(checktime AS DATE) BETWEEN '$date1' AND '$Date2' GROUP BY CAST(checktime AS DATE)  
 )  
    ...
    AND ID IN (
SELECT MAX(id) FROM checkinout WHERE userid='$use' AND  CAST(checktime AS TIME) 
BETWEEN '$pmini' AND '$pminf' AND checktype='$ckpmi'  AND CAST(checktime AS DATE) 
BETWEEN '$date1' AND '$Date2' GROUP BY CAST(checktime AS DATE)  
 )   
    THEN TIME_FORMAT(CAST(g.checktime AS TIME), '%h:%i') ELSE '' END ) AS Clock3,
      ...
      AND ID IN (
SELECT MAX(id) FROM checkinout WHERE userid='$use' AND  CAST(checktime AS TIME) 
BETWEEN '$pmouti' AND '$pmoutf' AND checktype='$ckpmo'   AND CAST(checktime AS DATE) 
BETWEEN '$date1' AND '$Date2' GROUP BY CAST(checktime AS DATE)  
 )
...
<table width="50%">
    ...
    <tr>
        <td>Civil&nbsp;Service&nbsp;Form&nbsp;No.&nbsp;48</td>
        <td>'.str_repeat("&nbsp;",200).'</td>
    </tr>
</table>
<br>
';
...
<th>-----o0o-----</th>
        <tr>
<td width="25%">'.str_repeat("&nbsp;",25).'<u>'.$name.'</td>
<tr>
<td>'.str_repeat("&nbsp;",30).'(Name)</td>
<tr>
</table>
';
 ...
 <br>
 <p style="padding-left: 120px; font-family:arial; font-size=19px">'.str_repeat("&nbsp;",25).'-----o0o-----</p>
<br>
...
';
    ...
    <p style="text-align:justify; font-family:arial; font-size: 13px;">'.str_repeat("&nbsp;",5).'I  CERTIFY <i> on <i> my <i> honor <i> that <i> the <i> above <i> is <i> a  true <i> and
    <br>correct <i> report <i> of <i> the hours of work <i> performed, <i> record <i> of
    <br>which was made daily at
	  the time <i> of arrival <i> and <i> departure <br> from office.</i> </p>
